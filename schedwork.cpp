...
)
{
    ...
    std::vector<size_t> workingDays(availability[0].size(), 0);
...
bool ifworkerassignedtoday(Worker_T workerId,DailySchedule& dailySched,size_t currentDay){
...
)
{
    ...
    // If the current day's schedule is not yet filled, try to add workers
        ...
        for (Worker_T workerId = 0; workerId < availability[0].size(); workerId++) {
            ...
            //     }
            // }
            alreadyAssignedToday=ifworkerassignedtoday(workerId,dailySched,currentDay);
            // If the worker is available, hasn't reached max shifts, and is not already assigned, add them to the schedule
            bool x = false;
            if (availability[currentDay][workerId] == 1 && workingDays[workerId] < maxShiftsAllowed && !alreadyAssignedToday) {
